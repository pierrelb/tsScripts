...
# issues with the triange smoothing, so decrease the 'infinite' distances
...
if family.lower() == 'h_abstraction':
    ...
    rBM[lbl3][lbl1] -= 0.7
    rBM[lbl3][lbl2] -= 0.7
    rBM[lbl1][lbl3] = rBM[lbl3][lbl1] + 0.1
    rBM[lbl2][lbl3] = rBM[lbl3][lbl2] + 0.1
    pBM[lbl3][lbl1] -= 0.7
    pBM[lbl2][lbl1] -= 0.7
    pBM[lbl1][lbl3] = pBM[lbl3][lbl1] + 0.1
    pBM[lbl1][lbl2] = pBM[lbl2][lbl1] + 0.1

rdkit.DistanceGeometry.DoTriangleSmoothing(rBM)
rdkit.DistanceGeometry.DoTriangleSmoothing(pBM)

# rGeom = Geometry(quantumMechanics.settings, 'reactant', reactant, rMult)
# pGeom = Geometry(quantumMechanics.settings, 'product', product, pMult)
# 
# rGeom.generateRDKitGeometries(rBM)
# pGeom.generateRDKitGeometries(pBM)
